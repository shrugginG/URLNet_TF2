...
def tokenizer(url):
    return TOKENIZER_RE.findall(url)


def build_word_vocab(urls, min_word_freq=0):
    ...
    filtered_word_counter = {
        word: freq for word, freq in word_counter.items() if freq > min_word_freq
    }
    # sorted_words = sorted(
    #     filtered_word_counter.items(), key=lambda item: item[1], reverse=True
    # )
    word_freq_dict = {word: freq for word, freq in filtered_word_counter.items()}
    # print(word_freq_dict)
    vocab_dict = {
        i + 1: word for i, (word, _) in enumerate(filtered_word_counter.items())
    }
    # print(vocab_dict)
    with open("/home/jxlu/project/PhishDetect/URLNet/data/word_vocab.txt", "w") as f:
        for word in vocab_dict.values():
            f.write(word + "\n")
    return vocab_dict, word_freq_dict


# Applies to TensorFLow2
def get_word_vocab(urls, max_length_words, min_word_freq=0):

    build_word_vocab(urls, min_word_freq)

    # The custom split function of vectorize_layer is not work
    # Split the origin urls manually
    urls = [" ".join(tokenizer(url)) for url in urls]
    vectorize_layer = tf.keras.layers.TextVectorization(
        ...
        vocabulary="/home/jxlu/project/PhishDetect/URLNet/data/word_vocab.txt",
    )

    word_vocab = vectorize_layer.get_vocabulary()
    index_word_vocab = {i - 1: word for i, word in enumerate(word_vocab)}
    del index_word_vocab[-1]
    model = tf.keras.models.Sequential()
    model.add(tf.keras.Input(shape=(1,), dtype=tf.string))
    model.add(vectorize_layer)
    x = np.array(
        [[i if i == 0 else i - 1 for i in array] for array in model.predict(urls)]
    )
    return x, index_word_vocab
    ...
    for url in word_x:
        ...
        for word in words:
            ...
            if (len(ngrams) > max_len_subwords) or (
                high_freq_words is not None
                and len(word) > 1
                and not is_in(high_freq_words, word)
            ):
...
if __name__ == "__main__":
    ...
    urls, labels = read_data("./data/dataset/tmp.csv")
    x, word_reverse_dict = get_word_vocab(urls, 200)
    print(x)
    print(word_reverse_dict)
    word_x = get_words(x, word_reverse_dict, 1, urls)
    print(word_x)
    ngramed_id_x, ngrams_dict, worded_id_x, words_dict = ngram_id_x(word_x, 20, None)
    print(ngramed_id_x)
    print(ngrams_dict)
    print(worded_id_x)
    print(words_dict)
    ...
    x_train, y_train, x_test, y_test = prep_train_test(pos_x, neg_x, 0.001)
