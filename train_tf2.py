...
if FLAGS["data.min_word_freq"] > 0:
    x1, word_reverse_dict = get_word_vocab(
        urls, FLAGS["data.max_len_words"], FLAGS["data.min_word_freq"]
    )
    high_freq_words = sorted(list(word_reverse_dict.values()))
    print(
        "Number of words with freq >={}: {}".format(
...
#     if shuffle:
#         dataset = dataset.shuffle(buffer_size=len(batch_data))
...
#     True,
# )
# print(nb_batches_per_epoch, nb_batches)

# for i in train_batches:
#     print(i)
#     # x_batch, y_batch = prep_batches(i)
#     # print(x_batch)
#     # print(y_batch)
#     break
...
def train_step(model, optimizer, x, y):
    with tf.GradientTape() as tape:
        scores, predictions = model(x, training=True)
...
@tf.function
def dev_step(model, x, y):
    scores, predictions = model(x, training=False)
    loss = model.compute_loss(scores, y)
    accuracy = model.compute_accuracy(predictions, y)
    return loss, accuracy
...
def run_training():
    ...
    optimizer = tf.keras.optimizers.Adam(FLAGS["train.lr"])
    ...
    batchs_index = tqdm(
        range(nb_batches),
        desc=f'emb_mode {FLAGS["model.emb_mode"]} delimit_mode {FLAGS["data.delimit_mode"]} train_size {x_train.shape[0]}',
        ncols=0,
    )
    ...
    for batch_index in batchs_index:
        ...
        loss, acc = train_step(cnn, optimizer, x_batch, y_batch)
        if batch_index % FLAGS["log.print_every"] == 0:
            print(f"Step {batch_index}, Loss: {loss:.4f}, Accuracy: {acc:.4f}")
            with open(train_log_dir, "a") as f:
                f.write(
                    f"{batch_index},{datetime.datetime.now().isoformat()},{loss:.4f},{acc:.4f}\n"
                )

        if batch_index % FLAGS["log.eval_every"] == 0 or batch_index == (
            nb_batches - 1
        ):
            total_dev_loss = 0.0
            nb_corrects = 0
            ...
            for test_batch in test_batches:
                ...
                dev_loss, dev_acc = dev_step(cnn, x_test_batch, y_test_batch)
                nb_instances += x_test_batch[0].shape[0]
                total_dev_loss += dev_loss * x_test_batch[0].shape[0]
                nb_corrects += dev_acc * x_test_batch[0].shape[0]

            dev_loss = total_dev_loss / nb_instances
            dev_acc = nb_corrects / nb_instances
            print(
                f"Eval: step {batch_index}, Loss: {dev_loss:.4f}, Accuracy: {dev_acc:.4f}"
            )
            ...
            if batch_index % FLAGS["log.checkpoint_every"] == 0 or batch_index == (
                nb_batches - 1
            ):
                if dev_loss < min_dev_loss:
                    checkpoint_manager.save()
                    # path = saver.save(sess, checkpoint_prefix, global_step=batch_index)
                    # min_dev_loss = dev_loss

    # for epoch in range(FLAGS["train.nb_epochs"]):
    #     print(f"Epoch {epoch + 1}/{FLAGS['train.nb_epochs']}")
    #     for step in tqdm(range(nb_batches_per_epoch)):
    #         batch = next(train_batches)
    #         x_batch, y_batch = prep_batches(batch)
    #         loss, acc = train_step(cnn, optimizer, x_batch, y_batch)
    ...
    #     total_dev_loss = 0.0
    #     total_dev_acc = 0.0
    #     for step in range(len(x_test) // FLAGS["train.batch_size"]):
    #         batch = next(test_batches)
    #         x_batch, y_batch = prep_batches(batch)
    #         dev_loss, dev_acc = dev_step(cnn, x_batch, y_batch)
    #         total_dev_loss += dev_loss
    #         total_dev_acc += dev_acc
    #     print(len(x_test))
    #     avg_dev_loss = total_dev_loss / (len(x_test) // FLAGS["train.batch_size"])
    #     avg_dev_acc = total_dev_acc / (len(x_test) // FLAGS["train.batch_size"])

    #     print(f"Validation Loss: {avg_dev_loss:.4f}, Accuracy: {avg_dev_acc:.4f}")

    #     if avg_dev_loss < best_dev_loss:
    #         best_dev_loss = avg_dev_loss
    #         best_dev_acc = avg_dev_acc
    #         cnn.save_weights(os.path.join(FLAGS["log.output_dir"], "best_model"))
...
#                 FLAGS["model.emb_mode"], FLAGS["data.delimit_mode"], x_train.shape[0]
#             ),
#             ncols=0,
#         )
#         for idx in it:
...
#                     f.write(
#                         "{:d},{:s},{:e},{:e}\n".format(
#                             step, datetime.datetime.now().isoformat(), loss, acc
#                         )
#                     )
#                 it.set_postfix(
...
#                     f.write(
#                         "{:d},{:s},{:e},{:e}\n".format(
#                             step, datetime.datetime.now().isoformat(), dev_loss, dev_acc
#                         )
#                     )
